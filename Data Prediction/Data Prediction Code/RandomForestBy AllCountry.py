...
df = pd.read_csv(root)
# 處理2028年缺失值
df = df[~df['year'].isin([2024,2028])]
# df = df[df['year'] != 2024]
# df = df[df['year'] != 2028]
# One-Hot 编码 'country' 列
...
for country, medals in zip(countries, predicted_medals_2028):
    print(f"Predicted total medals for {country} in 2024: {medals:.2f}")
...
plt.title('Predicted Total Medals for 2024 Olympics')
plt.show()